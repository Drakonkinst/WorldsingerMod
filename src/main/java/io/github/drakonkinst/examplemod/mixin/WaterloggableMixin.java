...
public interface WaterloggableMixin {
  ...
  default boolean tryFillWithFluid(WorldAccess world, BlockPos pos, BlockState state,
    ...
    if (state.contains(Fluidlogged.PROPERTY_FLUID) && !state.get(Properties.WATERLOGGED) &&
        state.get(Fluidlogged.PROPERTY_FLUID) == 0) {
      if (!world.isClient()) {
        ...
        world.setBlockState(pos, newState.with(Fluidlogged.PROPERTY_FLUID, index),
            Block.NOTIFY_ALL);
  ...
  default ItemStack tryDrainFluid(WorldAccess world, BlockPos pos, BlockState state) {
    if (state.get(Properties.WATERLOGGED) ||
        (state.contains(Fluidlogged.PROPERTY_FLUID) && state.get(Fluidlogged.PROPERTY_FLUID) > 0)) {
